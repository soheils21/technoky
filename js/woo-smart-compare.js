...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $('.woosc-settings-field-li').arrangeable({dragSelector: '.label', dragEndEvent: 'woosc_drag_settings',})
    });
    ...
    $(document).on('found_variation', function (e, t) {
        ...
        $('.woosc-btn-' + variable_id).removeClass('woosc-btn-added woosc-added').attr('data-id', t.variation_id);
    ...
    $(document).on('reset_data', function (e) {
        var variable_id = $(e.target).attr('data-product_id');
        $('.woosc-btn-' + variable_id).removeClass('woosc-btn-added woosc-added').attr('data-id', variable_id);
        if (woosc_vars.button_text_change === 'yes') {
            $('.woosc-btn-' + variable_id).html(woosc_vars.button_text)
        }
    });
    ...
    $(document).on('click touch', '#woosc-area .woosc-bar-item-remove', function (e) {
        var product_id = $(this).attr('data-id');
    ...
    function woosc_search() {
        ...
        woosc_timer = null;
        var data = {action: 'woosc_search', keyword: $('#woosc_search_input').val(), nonce: woosc_vars.nonce,};
    ...
    function woosc_add_product(product_id) {
        ...
        if (woosc_get_cookie(woosc_cookie_products) != '') {
            ...
            if (woosc_products.length < woosc_vars.limit) {
                ...
                woosc_limit = !0;
                woosc_limit_notice = woosc_limit_notice.replace('{limit}', woosc_vars.limit)
            }
    ...
    function woosc_load_bar(open) {
        var data = {action: 'woosc_load_bar', products: woosc_get_products(), nonce: woosc_vars.nonce,};
        $.post(woosc_vars.ajaxurl, data, function (response) {
    ...
    function woosc_open_bar() {
        ...
        $('.woosc-bar-item').arrangeable({dragSelector: 'img', dragEndEvent: 'woosc_drag_end_event',});
    ...
    function woosc_load_table() {
        ...
        var data = {action: 'woosc_load_table', products: woosc_get_products(), nonce: woosc_vars.nonce,};
        $.post(woosc_vars.ajaxurl, data, function (response) {
            ...
            if ($(window).width() >= 768) {
                if ((woosc_vars.freeze_column == 'yes') && (woosc_vars.freeze_row == 'yes')) {
                    $('#woosc_table').tableHeadFixer({'head': !0, left: 1})
                } else if (woosc_vars.freeze_column == 'yes') {
                    $('#woosc_table').tableHeadFixer({'head': !1, left: 1})
                } else if (woosc_vars.freeze_row == 'yes') {
                    $('#woosc_table').tableHeadFixer({'head': !0})
                }
            } else {
